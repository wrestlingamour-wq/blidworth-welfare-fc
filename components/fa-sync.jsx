...
function parseTable(text) {
  ...
  for (const raw of lines) {
    ...
    rows.push({
      pos, team: norm, p, w, d, l,
      gf: gf ?? 0, ga: ga ?? 0, gd: gd ?? 0, pts,
...
function parseFixtures(text, defaultYear) {
  ...
  for (const raw of lines) {
    ...
    let tokens = raw.includes('\t')
      ? raw.split('\t').map(s => s.trim()).filter(Boolean)
      : raw.split(/\s{2,}|(?<=\d)\s(?=[A-Z])/).map(s => s.trim()).filter(Boolean);
    ...
    if (timeMatch) { time = `${timeMatch[1].padStart(2,'0')}:${timeMatch[2]}`; cursor = 1; }
...
function diffSummary(oldArr, newArr, keyFn) {
  ...
  return { added: added.length, removed: removed.length, total: newArr.length };
}

// ─── EXPORTED COMPONENT ─────────────────────────────────────────────────────
function FASyncPanel({ onClose }) {
  ...
  const runParse = () => {
    ...
    const { fixtures: newFix, results: newRes } = fixturesText.trim()
      ? parseFixtures(fixturesText, currentYear)
      : { fixtures: null, results: null };
  ...
  const apply = () => {
    ...
    if (preview.fixtures?.length) {
      const upcoming = preview.fixtures
      ...
      if (upcoming) {
        ...
        BW_DATA.nextMatch = {
          ...
          home: isHome
            ...
            : { short: teamAbbr(upcoming.home), name: upcoming.home },
          away: !isHome
            ? { ...BW_DATA.nextMatch.home }
            : { short: teamAbbr(upcoming.away), name: upcoming.away },
    ...
    if (preview.results?.length) {
      ...
      if (latest) {
        BW_DATA.lastMatch = {
          ...BW_DATA.lastMatch,
          home: { short: teamAbbr(latest.home), name: latest.home, score: latest.hs },
          away: { short: teamAbbr(latest.away), name: latest.away, score: latest.as },
          date: latest.date,
          scorers: [],
          comp: latest.comp,
        };
  ...
  return (
    <div className="fa-sync-modal">
      <div className="fa-sync-inner">
        <div className="fa-sync-head">
          <div>
            <div className="eyebrow" style={{ color: 'var(--gold)' }}>FA Full-Time Sync</div>
            <h3 className="h-display" style={{ fontSize: 24, marginTop: 4 }}>
              Paste league data
            </h3>
          </div>
        ...
        <p style={{ fontSize: 13, lineHeight: 1.6, opacity: 0.75, marginBottom: 20 }}>
          Open the FA Full-Time page, select the table rows or fixture list with your mouse,
          copy, and paste below. Whitespace-separated columns — any format works.
        </p>

        <div className="fa-grid">
          <div>
            <label className="fa-label">League Table</label>
            <textarea
          ...
          <div>
            <label className="fa-label">Fixtures &amp; Results</label>
            <textarea
              className="fa-textarea"
        ...
        <div className="fa-actions">
          <button className="fa-btn fa-btn-ghost" onClick={runParse}
                  disabled={!tableText.trim() && !fixturesText.trim()}>
            Parse &amp; preview
          </button>
          {preview && (
            <button className="fa-btn fa-btn-gold" onClick={apply} disabled={applied}>
              {applied ? 'Applied — reloading…' : 'Apply to site'}
            </button>
          )}
          {preview && (
            <button className="fa-btn fa-btn-ghost" onClick={() => exportJSON(preview)}>
              ⬇ Export data.json
            </button>
          )}
        </div>
        ...
        {preview && (
          <div className="fa-preview">
            ...
            {preview.table?.length === 0 && preview.fixtures?.length === 0 && preview.results?.length === 0 && (
...
function PreviewRow({ label, summary, ok }) {
  return (
    <div className="fa-preview-row">
      <span className={`fa-badge ${ok ? 'fa-ok' : 'fa-fail'}`}>{ok ? '✓' : '✕'}</span>
      <strong style={{ width: 80, fontFamily: 'var(--font-display)', textTransform: 'uppercase', fontSize: 11, letterSpacing: '0.1em' }}>{label}</strong>
...
(function hydrate() {
  ...
  try {
    ...
    const upcoming = (BW_DATA.fixtures || [])
      .filter(f => new Date(f.date).getTime() >= today)
    ...
    if (upcoming) {
      ...
      BW_DATA.nextMatch = {
        ...BW_DATA.nextMatch,
        home: isHome
          ? { short: 'BWFC', name: 'Blidworth Welfare', crest: 'assets/crest.png' }
          : { short: abbr, name: upcoming.home },
        away: isHome
          ? { short: abbr, name: upcoming.away }
          : { short: 'BWFC', name: 'Blidworth Welfare', crest: 'assets/crest.png' },
        kickoff: `${upcoming.date}T${upcoming.time}:00+01:00`,
        venue: isHome ? 'Welfare Ground' : 'Away',
      };
    ...
    if (latestResult) {
      BW_DATA.lastMatch = {
        ...BW_DATA.lastMatch,
        home: { short: latestResult.home.split(/\s+/).map(w=>w[0]).join('').slice(0,3).toUpperCase(), name: latestResult.home, score: latestResult.hs },
        away: { short: latestResult.away.split(/\s+/).map(w=>w[0]).join('').slice(0,3).toUpperCase(), name: latestResult.away, score: latestResult.as },
